  ...
  ctx
    ...
    .action(async ({ session, options }) => {
      ...
      if (location) {
        ...
        let airPollutantData: Record<"pm10" | "pm2p5" | "co" | "no" | "no2" | "so2" | "o3" | "nmhc", number> | null =
          null;
        if (w_data.air) {
          // 找code是AQI的空气质量指数
          const air_quality = w_data.air.indexes.find((item) => item.code === "cn-mee") || w_data.air.indexes[0];
