  ...
  setup(__props) {
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (__props.now && __props.days && __props.days[0]) {
        ...
        if (__props.now.obsTime)
          _push(`<p class="update-time">${ssrInterpolate(__props.now.obsTime.split("T")[1].substring(0, 5))} 更新</p>`);
        else _push(`<!---->`);
        _push(
          `</div><div class="current-temp"><div class="temp-display">${ssrInterpolate(__props.now.temp)}°</div><div class="weather-desc"><em class="${ssrRenderClass(`qi-${__props.now.icon} weather-icon-large`)}"></em><span>${ssrInterpolate(__props.now.text)}</span></div></div><div class="temp-range"><span>最高 ${ssrInterpolate(__props.days[0].tempMax)}°</span><span class="separator">|</span><span>最低 ${ssrInterpolate(__props.days[0].tempMin)}°</span></div></div>`
        );
      ...
      if (__props.warning && __props.warning.alerts && __props.warning.alerts.length > 0) {
        ...
        ssrRenderList(__props.warning.alerts, (w) => {
          _push(
            `<div class="alert-card"><div class="alert-header"><em class="${ssrRenderClass(`qi-${w.icon} alert-icon`)}"></em><div class="alert-title"><h3>${ssrInterpolate(w.headline)}</h3><p class="alert-time">${ssrInterpolate(w.issuedTime)}</p></div></div><p class="alert-description">${ssrInterpolate(w.description)}</p></div>`
          );
        });
        _push(`<!--]--></div>`);
      } else _push(`<!---->`);
