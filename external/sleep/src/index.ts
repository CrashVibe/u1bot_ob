  ...
  ctx.command("我的作息", "查看个人作息统计").action(async ({ session }) => {
    ...
    return (
      ...
      (`你的个人作息统计\n` +
        ...
        `╠═══════════\n` +
        `║ 累计睡眠:\n` +
        `║  总时长: ${totalSleepHours.toString().padStart(3)}小时${totalSleepMinutes.toString().padStart(2)}分\n` +
        `╠═══════════\n` +
        `║ 本周数据:\n` +
        `║  早安次数: ${stats.weekly_morning_count.toString().padStart(5)}\n` +
        `║  晚安次数: ${stats.weekly_night_count.toString().padStart(5)}\n` +
        `║  睡眠总时: ${weeklySleepHours.toString().padStart(3)}小时${weeklySleepMinutes.toString().padStart(2)}分\n` +
        `║  最早早安: ${
