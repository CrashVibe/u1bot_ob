...
export async function apply(ctx: Context, config: Config) {
  applyModel(ctx);
  ctx
    .command("add_cave <content:text>", "添加内容到回声洞")
    .alias("投稿")
    .alias("回声洞投稿")
  ...
  ctx
    .command("add_anonymous_cave <content:text>", "匿名添加内容到回声洞")
    .alias("匿名投稿")
    .alias("匿名回声洞投稿")
    .action(async ({ session }, content) => {
      if (!content || !session) {
  ...
  ctx
    .command("cave [id:number]", "随机查看一个回声洞秘密")
    .alias("回声洞")
    .action(async (_, id) => {
